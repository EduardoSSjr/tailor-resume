...
O nome de um `\project{}` é um rótulo que o próprio usuário deu ao projeto (diferente de `\role{cargo}`, que é o título atribuído pelo empregador e por isso é sempre imutável). Por isso, ajustá-lo para o vocabulário da vaga é permitido — mas só sob estas regras:
...
- **Modo completo (`tailor-resume-full`)**: pode propor um alias novo, desde que ele continue descrevendo fielmente o mesmo projeto usando termos que já são verdadeiros dele — nunca ampliando escopo (ex: "Automação de Relatórios Financeiros" → "Automação de Relatórios Financeiros com Python" é válido, se o projeto de fato usa Python; já "→ Plataforma de Business Intelligence" não seria, porque amplia o escopo além do que o projeto realmente é). Pergunte antes de aplicar, junto com as perguntas do passo 5, e registre no passo 12.
- **Três travas, sempre**: (i) todo alias é ancorado no nome como aparece em `<arquivoCurriculo>` — nunca alias de alias; (ii) o match com o registro é **literal**, string por string — qualquer variação não listada exige nova pergunta, mesmo que pareça equivalente; (iii) isso vale **só** para nomes de projeto, nunca para cargo, empresa, datas ou formação.
...
6. **Reescreva as seções editáveis** com foco na vaga:
   ...
   - **Incorporando fatos confirmados no passo 5**: eles valem como qualquer outro fato das fontes de verdade, mas exigem cuidado extra para não parecerem colados especificamente para esta vaga. Priorize encaixar o fato dentro de um bullet ou enumeração já existente que combine com ele (ex: uma linguagem nova entra na lista de "Conhecimentos em ..." já existente, não vira uma linha própria) — só crie um bullet novo quando não houver nenhum item existente onde o fato caiba naturalmente. Em qualquer caso, escreva no mesmo registro seco e direto dos bullets vizinhos: sem parênteses explicando o contexto da resposta (ex: "faculdade, curso técnico e projetos pessoais"), sem cláusulas do tipo "em ambiente X e Y". Um trecho novo baseado no passo 5 não deve ser identificável, pela redação, como diferente do resto do currículo-base.
